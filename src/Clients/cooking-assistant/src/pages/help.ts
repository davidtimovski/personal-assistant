...
export class Help {
  ...
  async dismiss(tooltip: Tooltip) {
    ...
    await this.tooltipsService.toggleDismissed(tooltip.key, "CookingAssistant", true);
  }

  async retain(tooltip: Tooltip) {
    tooltip.isDismissed = false;
    await this.tooltipsService.toggleDismissed(tooltip.key, "CookingAssistant", false);
  }
}
