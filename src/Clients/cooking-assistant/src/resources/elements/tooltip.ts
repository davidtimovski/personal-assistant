...
export class TooltipCustomElement {
  ...
  async attached() {
    this.tooltip = await this.tooltipsService.getByKey(this.key, "CookingAssistant");
  ...
  async dismiss() {
    ...
    await this.tooltipsService.toggleDismissed(this.tooltip.key, "CookingAssistant", true);
  }
}
