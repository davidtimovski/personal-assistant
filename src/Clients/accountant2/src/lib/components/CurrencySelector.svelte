...
<div class="currency-selector-wrap">
	...
	<div class="currency-selector" class:changing>
		...
		/>
		<button type="button" id="selected-currency" on:click={toggleChangeCurrency} class="selected-currency"
			>{currency}</button
		>
	</div>
</div>
	...
	.currency-selector-wrap {
		...
		.currency-selector {
			...
			.selected-currency {
				...
				cursor: pointer;
				color: #fff;
				transition: background var(--transition-quick), border-radius var(--transition-quick),
