...
export class HttpProxy {
	...
	async ajax<T>(uri: string, init?: RequestInit): Promise<T> {
		...
		if (response.status === 204) {
			...
			throw new HttpProxyError(`GET request to '${uri}' returned 204 No Content`);
		}
	...
	async ajaxUploadFile(uri: string, init?: RequestInit): Promise<string | null> {
		init = this.setHeaders(init);
	...
	private async handleErrorCodes(response: Response): Promise<void> {
		...
		throw new HttpProxyError(`Error status code ${response.status} not handled`);
	}
}
