<script lang="ts">
	...
	let refreshButtonVisible = $state(false);
...
<div onclick={hide} class="alert {type}" class:shown>
	<span class="alert-body">
		<div class="alert-message" contenteditable="false" bind:innerHTML={message}></div>

		{#if refreshButtonVisible}
			<button type="button" onclick={refresh} class="refresh-button">{$t('refresh')}</button>
		{/if}
	</span>
</div>
