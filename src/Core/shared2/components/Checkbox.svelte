<script lang="ts">
	...
	let { labelKey, value, disabled = false }: { labelKey: string | null; value: boolean; disabled: boolean } = $props();
...
<label class="checkbox-toggle" class:disabled class:no-label={!labelKey}>
	...
	<span class="checkbox-toggle-content">
		<input type="checkbox" bind:checked={value} onchange={changeState} {disabled} />
