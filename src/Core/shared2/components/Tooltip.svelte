<script lang="ts">
	...
	let { key, application }: { key: string; application: string } = $props();
...
<div class="tooltip" class:visible={isVisible}>
	{#if tooltip}
		<div class="question-wrap">
			<button type="button" onclick={toggleOpen} class="question" class:glow={!isOpen}>{tooltip.question}</button>
		</div>

		{#if isOpen}
			<div in:slide class="answer-wrap">
				<div class="answer">
					<i class="fas fa-info-circle" class:faded={isDismissed}></i>
					<span>{tooltip.answer}</span>
					<div class="dismiss-wrap">
						<button type="button" onclick={dismiss} disabled={isDismissed}>{$t('tooltips.okayUnderstood')}</button>
					</div>
				</div>
			</div>
		{/if}
